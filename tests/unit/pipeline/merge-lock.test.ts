...
import type { MergeLockState } from "../../../src/pipeline/merge-lock.js";
...
describe("getMergeLockState", () => {
  test("returns null when no lock exists", () => {
    const state = getMergeLockState(dfDir);
    expect(state).toBeNull();
  });

  test("returns lock state when lock exists", () => {
    acquireMergeLock(dfDir, "run_001");
    const state = getMergeLockState(dfDir);
    expect(state).not.toBeNull();
    expect(state!.runId).toBe("run_001");
    expect(state!.pid).toBe(process.pid);
    expect(typeof state!.acquiredAt).toBe("string");
  });
...
describe("stale lock detection", () => {
  test("acquires lock when existing lock has dead PID", () => {
    ...
    const lockPath = join(dfDir, "merge.lock");
    const staleLock: MergeLockState = {
      runId: "run_stale",
    ...
    const state = getMergeLockState(dfDir);
    expect(state!.runId).toBe("run_new");
    expect(state!.pid).toBe(process.pid);
  });
  ...
  test("does not steal lock when PID is alive", () => {
    ...
    const lockPath = join(dfDir, "merge.lock");
    const activeLock: MergeLockState = {
      runId: "run_active",
      acquiredAt: new Date().toISOString(),
      pid: process.pid,
    };
...
describe("waitForMergeLock", () => {
  ...
  test("throws on timeout when lock is held", async () => {
    ...
    try {
      await waitForMergeLock(dfDir, "run_waiter", 100);
      // Should not reach here
      expect(true).toBe(false);
    } catch (err: any) {
      expect(err.message).toContain("Merge lock timeout");
      expect(err.message).toContain("run_holder");
    }
  });
  ...
  test("acquires after stale lock detected during wait", async () => {
    ...
    await waitForMergeLock(dfDir, "run_new", 5000);
