...
describe("Spec Editor Integration", () => {
  ...
  describe("Cannot edit completed spec (scenario: cannot-edit-completed-spec)", () => {
    test("PUT returns 403 for completed spec with completed run", async () => {
      seedSpec("spec_completed", "Completed Spec", "completed");
      // HEAD's isSpecLocked checks for completed runs, not just spec status
      ...
      db.prepare(
        `INSERT INTO runs (id, spec_id, status, skip_change_eval, max_parallel, budget_usd, max_iterations, config, created_at, updated_at)
         VALUES (?, ?, ?, 0, 4, 50.0, 3, '{}', ?, ?)`
      ).run("run_completed_1", "spec_completed", "completed", ts, ts);
      server = await startServer({ port: TEST_PORT, db });
  ...
  describe("Build from dashboard (scenario: build-from-dashboard)", () => {
    test("POST /api/builds creates run and updates spec status", async () => {
      ...
      // Verify spec still exists (HEAD implementation doesn't change spec status)
  ...
  describe("Build button disabled during active build (scenario: build-button-disabled-during-active-build)", () => {
    test("POST /api/builds returns 409 for spec with active run", async () => {
      seedSpec("spec_active", "Active Build", "building");
      // HEAD checks for active runs in the runs table, not spec status
      const ts = "2026-03-01T12:00:00Z";
      db.prepare(
        `INSERT INTO runs (id, spec_id, status, skip_change_eval, max_parallel, budget_usd, max_iterations, config, created_at, updated_at)
         VALUES (?, ?, ?, 0, 4, 50.0, 3, '{}', ?, ?)`
      ).run("run_active_1", "spec_active", "running", ts, ts);
      server = await startServer({ port: TEST_PORT, db, specsDir: join(tmpDir, "specs") });
    ...
    test("POST /api/builds returns 409 for spec with pending run", async () => {
      seedSpec("spec_pending", "Pending Spec", "draft");
      const ts = "2026-03-01T12:00:00Z";
      db.prepare(
        `INSERT INTO runs (id, spec_id, status, skip_change_eval, max_parallel, budget_usd, max_iterations, config, created_at, updated_at)
         VALUES (?, ?, ?, 0, 4, 50.0, 3, '{}', ?, ?)`
      ).run("run_pending_1", "spec_pending", "pending", ts, ts);
      server = await startServer({ port: TEST_PORT, db, specsDir: join(tmpDir, "specs") });

      const res = await fetch(`${server.url}/api/builds`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ specId: "spec_pending" }),
      });
      expect(res.status).toBe(409);
    });
  });
