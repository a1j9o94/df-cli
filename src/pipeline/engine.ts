...
export class PipelineEngine {
  ...
  /**
   * Resume a previously failed or stale run from the last completed phase.
   */
  async resume(options: ResumeOptions): Promise<string> {
    const { runId, fromPhase, budgetUsd } = options;
    ...
    // If currently "running", check for active agents
    if (run.status === "running") {
      const activeAgents = getActiveAgents(this.db, runId);
      const liveAgents = [];
      for (const agent of activeAgents) {
    ...
    updateRunStatus(this.db, runId, "running");
    createEvent(this.db, runId, "run-resumed" as any, { fromPhase: resumePhase });
    ...
    try {
      ...
      createEvent(this.db, runId, "run-failed", { error });
      log.error(`Pipeline failed: ${error}`);
      console.log(`[dark] Pipeline failed: ${error}. Do NOT attempt manual implementation.`);
    }
