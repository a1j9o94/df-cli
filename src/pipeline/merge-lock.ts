...
/**
 * Read and parse the lock file. Returns null if file doesn't exist or is invalid.
 */
export function getMergeLockState(dfDir: string): MergeLockState | null {
 ...
 *
 * - If no lock exists, creates one and returns true.
 * - If lock exists for the same runId, returns true (idempotent).
 * - If lock exists for a different runId with a dead PID, steals the lock and returns true.
 * - If lock exists for a different runId with a live PID, returns false.
 */
export function acquireMergeLock(dfDir: string, runId: string): boolean {
  const existing = getMergeLockState(dfDir);
  ...
  // Write new lock
  const state: MergeLockState = {
    runId,
    acquiredAt: new Date().toISOString(),
    pid: process.pid,
  };

  writeFileSync(lockPath(dfDir), JSON.stringify(state));
  return true;
}
 ...
 *
 * Polls at 1-second intervals. If the lock holder's PID is dead,
...
export async function waitForMergeLock(
  ...
  while (true) {
    ...
    if (elapsed >= timeoutMs) {
      throw new Error(
        `Merge lock timeout: could not acquire lock within ${timeoutMs}ms. Lock held by run ${getMergeLockState(dfDir)?.runId ?? "unknown"}.`,
      );
    }
...
export function getMergeQueuePosition(
  ...
  const lockState = getMergeLockState(dfDir);
  if (lockState?.runId === runId) {
    return 0;
  }
  ...
  try {
    ...
    // If the lock holder is among them, subtract 1 since they're actively merging
    let ahead = myIndex;
    if (lockState && rows.some((r) => r.id === lockState.runId)) {
