...
export const specCreateCommand = new Command("create")
  ...
  .action(async (title: string | undefined, options: CreateOptions) => {
    ...
    if (options.from) {
      if (!title) {
        log.error("Title is required when using --from. Usage: dark spec create <title> --from <spec-id>");
        process.exit(1);
      }
      await handleCreateFrom(title, options.from);
      return;
    }
...
async function handleCreateFrom(
  ...
  try {
    const spec = executeSpecCreateFrom(db, dfDir, title, parentSpecId);
    ...
    const commitResult = autoCommitFile(
      projectRoot,
      gitRelativePath,
      `Auto-commit: spec ${spec.id} created from ${parentSpecId}`,
    );
    if (commitResult.success) {
      log.info("  Spec committed to git history");
    }
